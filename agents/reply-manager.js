...
import { addSuppressed } from '../suppression/suppression-store.js';
...
export async function handleLeadReply(lead, inboundText) {
  ...
  if (['rejection', 'no_need'].includes(classification.intent)) {
    await addSuppressed({
      leadId: lead.id,
      businessName: lead.businessName,
      channel: lead.salesStrategy?.primaryChannel || lead.channel,
      target: lead.instagramHandle || lead.lineUrl || lead.websiteUrl || lead.businessName,
      reason: classification.intent,
      source: 'reply_manager'
    });
  }
